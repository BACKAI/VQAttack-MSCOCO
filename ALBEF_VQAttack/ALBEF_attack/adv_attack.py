...
if os.environ.get("VQATTACK_DISABLE_USE", "0") == "1":
    tf = None
    hub = None
else:
...
def _env_file_list(env_name, defaults):
...
def _load_json_merged(file_paths):
    merged = {}
    for file_path in file_paths:
        path = Path(file_path)
        if not path.exists():
            raise FileNotFoundError(f"Missing VQAttack asset file: {path}")
        with path.open("r", encoding="utf-8") as fp:
            merged.update(json.load(fp))
    return merged


def _load_id_list(file_paths):
    ids = []
    for file_path in file_paths:
        path = Path(file_path)
        if not path.exists():
            raise FileNotFoundError(f"Missing VQAttack asset file: {path}")
        with path.open("r", encoding="utf-8") as fp:
            ids.extend(int(line.strip()) for line in fp if line.strip())
    return ids
...
class Adv_attack:
    def __init__(self, vqa_model,pretrain_model,tokenizer,device,config):
        ...
        right_part_files = _env_file_list(
            "VQATTACK_RIGHT_PART_FILES",
            ["right_part.txt", "right_part_after.txt"],
        )
        vilt_ans_files = _env_file_list(
            "VQATTACK_VILT_ANS_FILES",
            ["vilt_ans_table_for_chatgpt.txt", "vilt_ans_table_for_chatgpt_after.txt"],
        )
        target_ans_files = _env_file_list(
            "VQATTACK_TARGET_ANS_FILES",
            ["albef_ans_table.txt", "albef_ans_table_after.txt"],
        )
        chatgpt_files = _env_file_list(
            "VQATTACK_CHATGPT_FILES",
            ["chatgpt_all_5k.txt", "chatgpt_all_5k_after.txt"],
        )
        all_correct_ans_files = _env_file_list(
            "VQATTACK_ALL_CORRECT_FILES",
            ["all_correct_ans.txt", "all_correct_ans_after.txt"],
        )

        self.adv_store_img_source = os.environ.get('VQATTACK_ADV_IMG_DIR', 'attack_dir') #store adversarial image
        self.adv_store_txt_source = os.environ.get('VQATTACK_ADV_TXT_PATH', 'adv_txt_dict_albef.txt') #store adversarial text
        self.adv_txt_dict = {}
        if not os.path.exists(self.adv_store_img_source):
            os.makedirs(self.adv_store_img_source)
        adv_txt_parent = Path(self.adv_store_txt_source).parent
        if str(adv_txt_parent) not in ("", "."):
            adv_txt_parent.mkdir(parents=True, exist_ok=True)
        if os.path.exists(self.adv_store_txt_source):
            try:
                with open(self.adv_store_txt_source, "r", encoding="utf-8") as fp:
                    self.adv_txt_dict = json.load(fp)
            except json.JSONDecodeError:
                self.adv_txt_dict = {}
        self.correct_list = _load_id_list(right_part_files)
        self.vilt_ans_table = _load_json_merged(vilt_ans_files)
        self.tcl_ans_table = _load_json_merged(target_ans_files)
        self.chatgpt = _load_json_merged(chatgpt_files)
        self.all_correct_ans = _load_json_merged(all_correct_ans_files)
        self.white_model=copy.deepcopy(pretrain_model)

        pretrain_checkpoint_path = os.environ.get("VQATTACK_ALBEF_PRETRAIN_CKPT", "").strip()
        if not pretrain_checkpoint_path:
            raise FileNotFoundError("Set VQATTACK_ALBEF_PRETRAIN_CKPT to the ALBEF pretrain checkpoint path.")
        ...
        finetune_checkpoint_path = os.environ.get("VQATTACK_ALBEF_VQA_CKPT", "").strip()
        if not finetune_checkpoint_path:
            raise FileNotFoundError("Set VQATTACK_ALBEF_VQA_CKPT to the ALBEF VQA fine-tuned checkpoint path.")
        ...
        self.disable_use = os.environ.get("VQATTACK_DISABLE_USE", "0") == "1"
        self.USE_model = None
        if not self.disable_use:
            use_path = os.environ.get("VQATTACK_USE_MODEL_DIR", "/tfhub_pretrained/universal-sentence-encoder-large_5")
            with tf.device('cpu'):
    ...
    def update_adv_text(self, text_embed_gradient, bert_cand_list, cand_wd_list, adv_text, attack_vector, ori_emb,
        ...
        for idx, (cand_wd_idx, sub_wd_idx) in enumerate(zip(cand_wd_list, attack_vector)):
            if cand_wd_idx >= len(words) or cand_wd_idx >= len(bert_cand_list):
                continue
            if sub_wd_idx >= text_embed_gradient.size(1) or sub_wd_idx >= ori_emb.size(1):
                continue
            ...
            for idd, cand_wd in enumerate(cand_list):
                ...
                if sub_wd_idx >= adv_text_embeds.size(1):
                    continue
        ...
        for (dir_sim, op) in zip(sorted_sim_list, sorted_op_list):
            ...
            if op[0] >= len(adv_words) or op[0] >= len(bert_cand_list):
                continue
            if not bert_cand_list[op[0]] or op[1] >= len(bert_cand_list[op[0]]):
                continue
            ...
            if self.disable_use or self.USE_model is None:
                USE_sim = 1.0
            else:
    ...
    def evaluate(
        ...
        import json
        print_freq=50000
        attack_limit = int(os.environ.get("VQATTACK_LIMIT", "0") or "0")
        for i, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
            if attack_limit and len(self.adv_txt_dict) >= attack_limit:
                break
            qid_key = str(int(batch['question_id'][0]))
            if int(qid_key) not in self.correct_list:
                continue
            adv_img_path = os.path.join(self.adv_store_img_source, qid_key + '.pt')
            if qid_key in self.adv_txt_dict and os.path.exists(adv_img_path):
                continue
            ret = dict()
            cali_ans=[i[0] for i in batch['answer']]
            cali_weight=[i[0].cpu().numpy() for i in batch['weight']]
            ret['preds'] = [self.tcl_ans_table[qid_key]]
            ...
            vilt_ans = self.vilt_ans_table[qid_key]
            all_correct_ans = self.all_correct_ans[qid_key]
            ans = copy.deepcopy(ret['preds'][0])
            self.batch = copy.deepcopy(attack_batch)
            paraphrase_text = self.chatgpt[qid_key][1]
            ...
            attack_pairs = [
                (word_idx, token_idx)
                for word_idx, token_idx in zip(sub_list, attack_vector)
                if token_idx < encoding["input_ids"].size(1)
            ]
            if attack_pairs:
                sub_list = [word_idx for word_idx, _ in attack_pairs]
                attack_vector = [token_idx for _, token_idx in attack_pairs]
            else:
                sub_list = []
                attack_vector = []
                iter_list = []
            ...
            if len(self.attack_dict) == 10:
                for qid_key in self.attack_dict.keys():
                    ...
                    topk_k = min(128, answer_input.input_ids.size(0))
                    topk_ids, topk_probs = self.black_model(image, question_input, answer_input, train=False, k=topk_k)
